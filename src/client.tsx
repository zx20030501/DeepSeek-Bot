...
function draftOf(settings: HermesBotSettings): Draft {
  const collaborationDefaults: FleetSettings = {
    ...
    maxParallelRuns: 6,
    botRunMaxAttempts: 3,
    features: { dynamicRegistry: false, chatBotCreation: false },
  }
    ...
    <section className="dsh-hermes-panel">
      ...
      <div className="dsh-hermes-dynamic-bots">
        ...
        {registryBots.length === 0 ? <span className="dsh-hermes-muted">暂无动态 Bot。启用对话创建后，可在飞书发送 <code>/bot create analyst 数据分析师</code>。</span> : registryBots.map(bot => {
          ...
          return <div className="dsh-hermes-fleet-row" key={botId || handle}><div><strong>@{handle} · {bot.title ?? handle}</strong><span>{bot.status} · {bot.fleetRole ?? 'generalist'} · v{String(bot.version ?? 1)} / r{String(bot.revision ?? 1)} · {bot.runtimeReady ? `已进入 roster（dynamic r${String(bot.runtimeRevision ?? bot.revision ?? 1)}）` : bot.blockedReason ?? '尚未运行'}</span></div><div className="dsh-hermes-action-row">
