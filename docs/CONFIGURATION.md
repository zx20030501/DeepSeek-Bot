...
- `/bot confirm <code>`、`/bot list`：确认激活或查看自己的动态 Bot；
...
阶段 2 已实现。两项开关可在本机设置页的“Bot Fleet 设置”中开启；保存后在飞书发送 `/new`。创建流程是“草稿 → 绑定具体 Revision 的 8 位确认码 → 激活”，未确认草稿不能运行；修改草稿会生成新码并废止旧码。聊天创建目前只开放用户私有作用域，并把“平台 + UID”写入不可由模型伪造的 principal ACL。静态 Bot、动态 Bot 和删除墓碑共用同一 handle 命名空间；网页保存、运行时重载和聊天创建使用同一命名空间事务，冲突或应用失败时不会保留半完成设置。

`peerMessaging`、`managerAgent`、`savedWorkflows` 和 `externalRuntimes` 仍未开放；即使手工打开尚未实现的开关，也不会赋予对应能力。
