...
- Workflow 的 condition/map/reduce/compensation 目前仅支持有界 task-only 运行时；approval、timeout、完整 retry policy、外部副作用和完整补偿语义仍未实现；
- 通用 DAG 编辑器、聊天 Workflow 管理命令和运行中动态阶段；
- `@team:<team-id>` 的跨机器 Team Transport 与分布式一致性（本机 canonical Team/Thread Router 已实现）；
