...
- `/teams`、`/team create`、`/team add`、`/team remove`、`/team manager`、`/team status`：显式管理 Team 成员；新 Bot 不会自动扩展既有 Team；
...
这是单机 DeepSeek Harness 内的受控 Fleet，不是 xAI 内部系统的复制品。聊天创建的 Bot 经用户确认后可自动加入用户私有 roster；确定性 Manager、`/fleet`、单机 `@team:<team-id>` Router 和已保存 Workflow 可以在 ACL 与预算内选择它。单次 Group Room/计划和同时运行的 Agent 仍受最多 6 Bot 等配置上限约束；“500 Bot”只表示逻辑 roster 数据路径测试，不表示 500 个模型会话并发。版本化 Workflow 与 task-node DAG 已实现，condition/map/reduce/compensation 已支持有界的 task-only 运行时；approval、timeout、完整 retry policy、外部副作用语义、通用 DAG 编辑器、跨机器 Transport、定时 Routine、完整文件/媒体转发和运行中任意改图仍未实现；`@team` 的跨机器路由和分布式一致性仍未实现。
