...
test('Bot model failure creates a fresh Run and really retries after backoff', async () => {
  ...
  try {
    ...
    const completedRun = fleet.runs.find(run => run.status === 'completed')
    const completedDelivery = (await gateway.mailbox.snapshot()).find(item => item.envelope.runId === completedRun?.id)
