...
test('pairing creates a bounded one-time code and approves the bound sender', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'deepseek-bot-pairing-'))
  try {
    ...
    assert.equal(await store.isApproved('telegram', 'ou_user'), false)
...
test('unknown direct messages create a pairing request without reaching the Agent', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'deepseek-bot-pairing-'))
